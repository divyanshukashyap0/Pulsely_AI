...
export default async function workoutRoutes(fastify: FastifyInstance) {
  ...
  // Complete workout
  fastify.patch('/:id/complete', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userId = request.user!.userId;
      const { id } = request.params as { id: string };
  ...
  fastify.post('/:workoutId/exercises', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userId = request.user!.userId;
  ...
  fastify.post('/sets', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userId = request.user!.userId;
