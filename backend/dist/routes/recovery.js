 ...
 */
async function calculateReadinessScore(userId, date) {
    ...
    if (recovery.sleepHours) {
        if (recovery.sleepHours >= 7 && recovery.sleepHours <= 9)
            sleepScore = 80;
        else if (recovery.sleepHours >= 6 && recovery.sleepHours <= 10)
            sleepScore = 60;
        else
            sleepScore = 40;
    }
    ...
    if (totalVolume > 10000)
        ...
        strainScore = 80;
    const overallScore = Math.round(sleepScore * 0.4 + fatigueScore * 0.3 + strainScore * 0.3);
